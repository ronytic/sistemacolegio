...
if (!empty($_GET) && $_GET['lock'] == md5('lock')) {
	...
	$curso = new curso;
	extract($_GET);
	$doc = $docente->mostrarRegistro($CodDocente);
	$doc = array_shift($doc);
	$relleno = isset($relleno) ? $relleno : 0;
	...
	foreach ($docentemateriacurso->mostrarDocenteGrupo($CodDocente, "") as $dmc) {
		...
		switch ($dmc['SexoAlumno']) {
			case 0: {
					$TextoSexo = $idioma['SoloMujeres'];
				}
				break;
			case 1: {
					$TextoSexo = $idioma['SoloVarones'];
				}
				break;
			case 2: {
					$TextoSexo = $idioma['AmbosSexos'];
				}
				break;
		}

...
if ($_GET['f'] != "") {
	...
	/*Primera Parte*/
	$x = -4 + (isset($_GET['x']) ? $_GET['x'] : 0);
	$y = 15 + (isset($_GET['y']) ? $_GET['y'] : 0);
...
$pdf->Cell(190,15,"",1);*/
	...
	$i = $y + 35;
	foreach ($facturadetalle->mostrarFacturaDetalles("CodFactura=" . $CodFactura) as $fd) {
		$i += 4;
		$pdf->SetXY($x + 25, $i);
	...
	celda(110, mayuscula(num2letras(number_format($f['TotalBs'], 2, ".", ''))) . " " . $idioma['Bolivianos'], "", "8");

...
10-05-2013
	*/
	...
	if ($ag['CodObservacion'] == 8) {
		$horaAtraso = $ag['HoraRegistro'];
	} else {
		$horaAtraso = "";
	}
	$mensaje = quitarTilde($idioma['Agenda']) . " $Sigla: " . quitarTilde(capitalizar($al['Paterno'])) . " " . quitarTilde(capitalizar(acortarPalabra($al['Nombres']))) . ", Obs: " . quitarTilde($obs['Nombre'], false) . " " . quitarTilde($ag['Detalle'], false) . ", " . quitarTilde($mat['Nombre'], false) . ", " . date("Y-m-d", strtotime($ag['Fecha'])) . " $horaAtraso ";
	$mensaje = ($mensaje);

...
?>
	...
	if (count($asis) > 0) {	?>
		<div class="alert alert-error grande"><?php echo $idioma['YaMarcoAsistencia'] ?></div>
		<?php
	} else {
		switch ($Dia) {
			case 1: {
					$HoraInicio = $cArea['HoraInicioL'];
					$HoraAtraso = $cArea['HoraEsperaL'];
				}
				break;
			case 2: {
					$HoraInicio = $cArea['HoraInicioM'];
					$HoraAtraso = $cArea['HoraEsperaM'];
				}
				break;
			case 3: {
					$HoraInicio = $cArea['HoraInicioMi'];
					$HoraAtraso = $cArea['HoraEsperaMi'];
				}
				break;
			case 4: {
					$HoraInicio = $cArea['HoraInicioJ'];
					$HoraAtraso = $cArea['HoraEsperaJ'];
				}
				break;
			case 5: {
					$HoraInicio = $cArea['HoraInicioV'];
					$HoraAtraso = $cArea['HoraEsperaV'];
				}
				break;
			case 6: {
					$HoraInicio = $cArea['HoraInicioS'];
					$HoraAtraso = $cArea['HoraEsperaS'];
				}
				break;
			case 7: {
					$HoraInicio = $cArea['HoraInicioD'];
					$HoraAtraso = $cArea['HoraEsperaD'];
				}
				break;
		}
...
//print_r($cArea);*/
	...
	?>
	<?php $ima = $folder . "imagenes/alumnos/" . $al['Foto'];
	...
	<table class="tabla">
		<tr>
			...
			<td colspan="3" class="text-info x2 espacio"><?php echo capitalizar($al['Paterno'] . " " . $al['Materno'] . " " . $al['Nombres']) ?></span></td>
		</tr>
		<tr>
			<td class="text-info x2 espacio"><?php echo $cur['Nombre'] ?></span></td>
		</tr>
	</table>
	<script language="javascript">
		$("#Codigo").val('').focus();
		mostrar();
		<?php if ($AtrasoAgenda && isset($CodAgenda)) {
		?>
			$.post('../../agenda/total/enviarsms.php', {
				'Codigo': <?php echo $CodAgenda; ?>
			}, function() {

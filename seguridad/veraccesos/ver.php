...
<table class="table">
	...
	foreach ($logu as $lu) {
		...
		switch ($lu['Nivel']) {
			case "1": {
					...
					$Foto = $folder . "imagenes/usuario/" . ($ul['Foto'] ?? '0.jpg');
					$Paterno = $ul['Paterno'] ?? '';
					$Materno = $ul['Materno'] ?? '';
					$Nombres = $ul['Nombres'] ?? '';
				}
			...
			case "3": {
					...
					$Foto = $folder . "imagenes/docentes/" . ($ul['Foto'] ?? '0.jpg');
					$Paterno = $ul['Paterno'] ?? '';
					$Materno = $ul['Materno'] ?? '';
					$Nombres = $ul['Nombres'] ?? '';
				}
			...
			case "6": {
					...
					$Foto = $folder . "imagenes/alumnos/" . ($ul['Foto'] ?? '0.jpg');
					$Paterno = $ul['Paterno'] ?? '';
					$Materno = $ul['Materno'] ?? '';
					$Nombres = $ul['Nombres'] ?? '';
				}
	...
	?>
		...
		<tr>
			...
			<td>
				<span class="label label-info"><strong><?php echo $tipousuario ?>:</strong></span>

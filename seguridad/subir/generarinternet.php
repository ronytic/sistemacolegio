...
if (!empty($_POST) && is_array($_POST['Tablas'])) {
	...
	$info['mysqlver'] = mysqli_get_server_info($bd->l);
...
EOT;
	foreach ($tablas as $tabla) {
		...
		if ($estructura) {
			...
			$respuesta = mysqli_query($bd->l, $consulta)
				or die("No se pudo ejecutar la consultaaa: " . mysqli_error($bd->l));
		...
		$respuesta = mysqli_query($bd->l, $consulta)
			or die("No se pudo ejecutar la consulta: " . mysqli_error($bd->l));
		$i = 0;
		while ($fila = mysqli_fetch_array($respuesta)) {
			$i++;
			...
			foreach ($columnas as $columna) {
				if (gettype($fila[$columna]) == "NULL") {
					...
					$values[] = "'" . mysqli_real_escape_string($bd->l, $fila[$columna]) . "'";
				}
			}
			if ($i == 400) {
				$i = 0;

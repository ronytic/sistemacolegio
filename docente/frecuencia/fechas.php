	...
	<div class="box-content">
		...
		<table class="table table-bordered table-condensed table-hover table-striped">
			<thead>
				<tr>
					<th>N</td>
					<th><?php echo $idioma['Dia'] ?></th>
					<th><?php echo $idioma['FechaAcceso'] ?></th>
					<th><?php echo $idioma['HoraAcceso'] ?></th>
				</tr>
			</thead>
			<?php
			$i = 0;

			foreach ($logusuario->mostrarUsoDocente($CodDocente) as $lu) {
				$i++;

			?>
				<tr class="contenido">
					<td class="der"><?php echo $i; ?></td>
					<td><?php echo diaSemana(date("N", strtotime($lu['FechaLog']))); ?></td>
					<td><?php echo date("d-m-Y", strtotime($lu['FechaLog'])); ?></td>
					<td class="der"><?php echo $lu['HoraLog']; ?></td>
				</tr>
			<?php
			}
			?>

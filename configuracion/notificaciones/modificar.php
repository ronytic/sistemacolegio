...
if (!empty($_POST['CodNotificaciones'])) {
	...
	$paraquienseleccionado = explode(",", $men['Usuarios']);
?>
		...
		<table class="table table-bordered table-striped">
			...
			<tr>
				<td><?php echo $idioma['TipoMensaje'] ?><br>
					<?php campo("Tipo", "select", array("1" => $idioma["Error"], "2" => $idioma["Advertencia"], "3" => $idioma["Comunicado"]), "span12", 1, "", 0, "", $men['Tipo']) ?>
				</td>
			</tr>
			<tr>
				<td><?php echo $idioma['ParaQuien'] ?><br>
					<select name="Usuarios[]" id="Usuario" multiple="multiple" required>
						<?php foreach ($paraquien as $pqk => $pqv) { ?>
							<option value="<?php echo $pqk ?>" <?php echo in_array($pqk, $paraquienseleccionado) ? 'selected' : '' ?>><?php echo $pqv ?></option>
						<?php } ?>
					</select>
				</td>
			</tr>
